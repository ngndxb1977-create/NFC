...
if not VEHICLE_CATALOG["2025"] and not VEHICLE_CATALOG["2026"]:
    ...
    <head>
        <meta charset="UTF-8">
        <title>{selected_name} - Vehicle EMI & Documentation Report</title>
        <style>
            body {{ font-family: Arial, sans-serif; padding: 24px; background-color: #f7f5f0; color: #191919; }}
            h1, h2, h3 {{ font-family: Arial, sans-serif; }}
            h1 {{ text-align: center; margin-bottom: 24px; }}
            h2 {{ margin-top: 32px; border-bottom: 2px solid #191919; padding-bottom: 4px; }}
            h3 {{ margin-top: 20px; }}
            .section {{ margin-bottom: 24px; padding: 16px; background-color: #ffffff; border-radius: 8px; }}
            .badge {{ display: inline-block; padding: 4px 10px; border-radius: 999px; background-color: #191919; color: #f7f5f0; font-size: 0.8rem; }}
            table {{ width: 100%; border-collapse: collapse; margin-top: 12px; }}
            th, td {{ padding: 8px; border-bottom: 1px solid #ddd; text-align: left; font-size: 0.9rem; }}
            th {{ background-color: #f0ede5; }}
            ul {{ margin-top: 8px; }}
            li {{ margin-bottom: 4px; }}
            img {{ max-width: 360px; display: block; margin: 16px auto; border-radius: 8px; border: 1px solid #ccc; }}
            .footer {{ margin-top: 40px; font-size: 0.8rem; text-align: center; color: #666; }}
        </style>
    </head>
    <body>

        <h1>Vehicle EMI & Application Summary Report</h1>
        <div style="text-align:center; margin-bottom: 10px;">
            <span class="badge">{selected_name}</span>
        </div>

        <!-- SECTION 1: VEHICLE OVERVIEW -->
        <div class="section">
            <h2>1. Vehicle Overview</h2>
            <h3>Selected Vehicle</h3>
            <p><strong>Model:</strong> {selected_name}</p>

            <h3>Vehicle Image</h3>
            {"<img src='" + str(st.session_state.get("vehicle_image_path", "")) + "' alt='Vehicle Image'>" if st.session_state.get("vehicle_image_path") else "<p><em>No vehicle image available.</em></p>"}
        </div>

        <!-- SECTION 2: EMI MATRIX -->
        <div class="section">
            <h2>2. Vehicle EMI Matrix</h2>
            <p>This section summarizes the EMI options calculated for the selected vehicle.</p>
            {pd.DataFrame(vehicle_emi_results).to_html(index=False)}
        </div>

        <!-- SECTION 3: KEY FINANCIAL SUMMARY (OPTIONAL PLACEHOLDER) -->
        <div class="section">
            <h2>3. Key Financial Summary</h2>
            <p><em>Fill in with your actual variables if available (e.g. loan amount, tenure, rate, etc.).</em></p>
            <ul>
                <li><strong>Example:</strong> Loan Amount, Tenure, Interest Rate, Monthly EMI, Total Payable.</li>
            </ul>
        </div>

        <!-- SECTION 4: APPLICATION NOTES (OPTIONAL PLACEHOLDER) -->
        <div class="section">
            <h2>4. Application Notes & Internal Remarks</h2>
            <p><em>You can inject any internal notes or risk comments here from your app logic.</em></p>
        </div>

        <!-- SECTION 5: DOCUMENTATION REQUIREMENTS & DISCLOSURES -->
        <div class="section">
            <h2>5. Application Requirements & Disclosures</h2>
            <div style="background-color: #F4F0EA; padding: 1.5rem; border-radius: 8px; border-left: 4px solid #191919; margin-top: 0.5rem;">
                <strong style="font-family: sans-serif; font-size: 1.05rem; color: #191919; display: block; margin-bottom: 0.75rem;">
        ...
        <div class="footer">
            <p>This report is system-generated based on the inputs and calculations performed in the NFC application.</p>
        </div>
    ...
    st.download_button(
        ...
        data=html_report,
        file_name=f"{selected_name.replace(' ', '_')}_Full_Report.html",
        mime="text/html",
        use_container_width=True
    )
